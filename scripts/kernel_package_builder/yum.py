...
def yum_update_upgrade(allow_errors=False, verbose=False, live_output=False):
    """
...
def yum_get_srpm(kernel_version, distro, allow_errors=False, verbose=True, builddir='./build'):
    ...
    if distro in elrepolist:
        return yum_get_elrepo_kernel_srpm(kernel_version, allow_errors, verbose, builddir)
    else:
        return yum_get_standard_kernel_srpm(kernel_version, allow_errors, verbose, builddir)


def yum_get_elrepo_kernel_srpm(kernel_version, allow_errors=False, verbose=True, builddir='./build'):
    """
    We will make a list of "ElRepo Distributions, and then use the LT version only for them.
    Download kernel rpm which gives us naming convention for downloading srpm
    """
...
def yum_get_standard_kernel_srpm(kernel_version, allow_errors=False, verbose=True, builddir='./build'):
    """
    We will make a list of "ElRepo Distributions, and then use the LT version only for them.
    Download kernel rpm which gives us naming convention for downloading srpm
...
def yum_get_kernel_tarball(filename, rpm_topdir, allow_errors=False, verbose=False):
...
def yum_upack_srpm(srpm_filename, allow_errors=False, verbose=False, builddir='./build'):
...
def yum_hack_elrepo_specfile(bitflux_version, pkg_release, patchfile_path, rpm_topdir, allow_errors=False, verbose=False):
...
def yum_hack_srpm_specfile(bitflux_version, pkg_release, patchfile_path, rpm_topdir, allow_errors=False, verbose=False):
    ...
    run_cmd(cmd, allow_errors=allow_errors, verbose=verbose)
    # Set bitflux version
    cmd = "sed -i '/buildid .local/a %define buildid 9{}.' {}".format(bitflux_version, specfile)
    run_cmd(cmd, allow_errors=allow_errors, verbose=verbose)
...
def get_package_yum(args):
    ...
    yum_update_upgrade(allow_errors=True, live_output=True)
    print("rpm repos updated and upgraded")
    sys.stdout.flush()
    sleep(2)

    srpm_filename = yum_get_srpm(None, args.distro, allow_errors=False, verbose=args.verbose)
...
def yum_style_build(args):
    # Update and upgrade apt repos to latest
    print("update and upgrade rpm repos...")
    yum_update_upgrade(allow_errors=True, live_output=True, verbose=args.verbose)
    ...
    srpm_filename = yum_get_srpm(args.kernel_version, args.distro, allow_errors=False, verbose=args.verbose)
    print("Found srpm name:            {}".format(srpm_filename))
    sys.stdout.flush()
    sleep(2)
    ...
    rpm_topdir = yum_upack_srpm(srpm_filename, allow_errors=False, verbose=args.verbose)

    tarball = yum_get_kernel_tarball(srpm_filename, rpm_topdir, allow_errors=False, verbose=args.verbose)
    ...
    if args.distro in elrepolist:
        specfile = yum_hack_elrepo_specfile(bitflux_version, pkg_release, patchfile_path, rpm_topdir, allow_errors=False, verbose=args.verbose)
    else:
        specfile = yum_hack_srpm_specfile(bitflux_version, pkg_release, patchfile_path, rpm_topdir, allow_errors=False, verbose=args.verbose)
    ...
    yum_generate_srpm_config_local(bitflux_version, pkg_release, patchfile_path, rpm_topdir, allow_errors=False, verbose=args.verbose)
