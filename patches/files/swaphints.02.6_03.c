...
static void swaphints_print_page_flags(struct page *page)
{
	...
	if (PageLocked(page))
		printk(KERN_ERR "PageLocked\n");
	if (PagePrivate(page))
		printk(KERN_ERR "PagePrivate\n");
	if (PagePrivate2(page))
		printk(KERN_ERR "PagePrivate2\n");
	if (PageWriteback(page))
		printk(KERN_ERR "PageWriteback\n");
	if (PageReserved(page))
		printk(KERN_ERR "PageReserved\n");
	if (PageSlab(page))
		printk(KERN_ERR "PageSlab\n");
	if (PageActive(page))
		printk(KERN_ERR "PageActive\n");
	if (PageUnevictable(page))
		printk(KERN_ERR "PageUnevictable\n");
	if (PageMlocked(page))
		printk(KERN_ERR "PageMlocked\n");
	if (PageLRU(page))
		printk(KERN_ERR "PageLru\n");
	if (PageAnon(page))
		printk(KERN_ERR "PageAnon\n");
	if (PageSwapCache(page))
		printk(KERN_ERR "PageSwapCache\n");
	if (PageSwapBacked(page))
		printk(KERN_ERR "PageSwapBacked\n");
	if (PageMappedToDisk(page))
		printk(KERN_ERR "PageMappedToDisk\n");
	if (PageReclaim(page))
		printk(KERN_ERR "PageReclaim\n");
	if (page->flags | PAGE_FLAGS_CHECK_AT_FREE)
		printk(KERN_ERR "Page Flags Check At Free\n");
	if (page->flags | __PG_MLOCKED)
		printk(KERN_ERR "Page Flags __PG_MLOCKED\n");
	if (page->flags | LRU_GEN_MASK)
...
static unsigned long swaphints_swap_a_page(unsigned long pagenumber)
{
	...
	if (!folio) {
		//put_page(page);
		return -ERR_SWAPHINTS_NOFOLIO;
	}

	//if (!folio_test_lru(folio))
	//	return -ERR_SWAPHINTS_TEST_LRU1;

	if (!folio_try_get(folio))
		return -ERR_SWAPHINTS_TRY_GET;

	if (unlikely(page_folio(page) != folio)) {
		//put_page(page);
		folio_put(folio);
		return -ERR_SWAPHINTS_FOLIO_MISMATCH;
	}
	//put_page(page);

	//if (!folio_test_lru(folio)) {
	//	folio_put(folio);
	//	return -ERR_SWAPHINTS_TEST_LRU2;
	//}

	return reclaim_page(folio);
}
...
static void swaphint_print_status(u64 pfn, unsigned long status)
{
	...
	case 0:
		status_string = "Retry";
		break;
	case -ERR_SWAPHINTS_ISOLATE_LRU:
		status_string = "Isolate LRU Failed";
		break;
	case -ERR_SWAPHINTS_UNEVICTABLE:
		status_string = "Unevictable";
		break;
	case -ERR_SWAPHINTS_TAIL:
		status_string = "Tail";
		break;
	case -ERR_SWAPHINTS_NOPAGE:
		status_string = "No Page";
		break;
	case -ERR_SWAPHINTS_NOFOLIO:
		status_string = "No Folio";
		break;
	case -ERR_SWAPHINTS_TRY_GET:
		status_string = "Try Get Failed";
		break;
	case -ERR_SWAPHINTS_FOLIO_MISMATCH:
		status_string = "Folio Mismatch";
		break;
	case -ERR_SWAPHINTS_TEST_LRU1:
		status_string = "Test LRU1 Failed";
		break;
	case -ERR_SWAPHINTS_TEST_LRU2:
		status_string = "Test LRU2 Failed";
		break;
	default:
		return;
	}

	printk(KERN_ERR "Swaphints failed to reclaim pfn %llu status=%lu %s\n", pfn, status, status_string);
}
